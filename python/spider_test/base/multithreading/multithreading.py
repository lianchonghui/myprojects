    ...
    def run(self):
        ...
        while not exitFlag_Parser:
            try:
                ...
                如果队列为空且block为True，get（）就使用线程暂停，直至有项目
        ...
        try:
            ...
            for site in result:
                try:
                    ...
                    with self.lock:
                        print 'write %s' % json.dump(result)
                        self.write(json.dump(result,ensure_ascii=False).encode('utf-8')+"\n")
...
def main():
    ...
    #初始化采集线程
    crawlThreads = []
    crawList = ['crawl-1','crawl-2','crawl-3']
