...
class SMSCodeHandler(BaseHandler):
    ...
    def post(self):
        ...
        piccode = self.json_args('piccode')
        ...
        # 手机好是否存在检查
        sql = 'select count(1) counts from ih_user_profile where up_mobili=%s'
        try:
        ...
        # 产生随机短信验证码
        sms_code = '%06d' % random.randint(1,10000000)
        try:
        ...
        # 发送短信验证码
        try:
            result = cpp.SendTemplateSMS(mobile,[sms_code,SMS_CODE_EXPIRES_SECONDS/60],1)
        except Exception as e:
            logging.error(e)
